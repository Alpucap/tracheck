...
export default function HistoryClient({ view, misses, flatData }: HistoryClientProps) {
    ...
    if (!isAuthenticated) {
        ...
        <main className="min-h-screen flex items-center justify-center bg-tracker-bg p-4">
            <div className="bg-white p-6 sm:p-8 rounded-xl shadow-xl border border-tracker-base w-full max-w-md animate-in fade-in zoom-in-95 duration-300">
            <div className="flex flex-col items-center mb-6 text-center">
                <div className="bg-tracker-light/30 p-4 rounded-full mb-4">
                <Lock className="text-tracker-dark" size={32} />
                </div>
                <h1 className="text-2xl font-bold text-tracker-dark">Administrative Access</h1>
                <p className="text-sm text-gray-500 mt-1">Halaman ini dilindungi. Masukkan password untuk melihat riwayat data.</p>
            </div>
            ...
            <form onSubmit={handleLogin} className="space-y-4">
                ...
                <input
                    ...
                    className={`w-full p-3 rounded-md border ${error ? 'border-red-500' : 'border-tracker-base'} outline-none focus:ring-2 focus:ring-tracker-dark bg-white text-gray-800`}
                    autoFocus
                />
                {error && <p className="text-xs text-red-500 font-bold">Password salah. Coba lagi.</p>}
                </div>
                <button
                type="submit"
                className="w-full bg-tracker-dark text-white py-3 rounded-md font-bold hover:bg-tracker-base transition-colors shadow-md active:scale-95"
                >
                UNLOCK HISTORY
                </button>
                <Link href="/" className="block text-center text-sm font-medium text-gray-400 hover:text-tracker-dark transition-colors pt-2">
    ...
    return (
        ...
        <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6 border border-tracker-light">
            ...
            <form className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-end flex-1">
                ...
                <button type="submit" className="bg-tracker-dark text-white px-6 py-2 rounded-md text-sm font-bold hover:bg-tracker-base transition-colors active:scale-95">FILTER</button>
            </form>
            </div>
